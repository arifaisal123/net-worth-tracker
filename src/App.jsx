...
import { 
  ...
  Wallet, Building, TrendingUp, X, Calendar, ChevronDown, ChevronUp, FileText, LogOut, Loader2, Lock, Mail
} from 'lucide-react';
          ...
          <div className="space-y-2">
            {payload.filter(p => p.dataKey !== 'total' && p.value > 0).map((entry, index) => (
              <div key={index} className="flex justify-between items-center text-sm">
                <span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }}></div>{entry.name}</span>
      ...
      return (
        <div className="bg-white p-3 border border-slate-200 rounded-xl shadow-lg text-sm z-50">
          <p className="font-semibold text-slate-700 flex items-center gap-2">
            <span className="w-3 h-3 rounded-full" style={{ backgroundColor: data.payload.color || data.payload.fill || '#10b981' }}></span>{data.name}
          </p>
          <p className="text-slate-900 font-bold mt-1 text-base">{formatCurrency(data.value)} <span className="text-slate-500 font-medium text-sm">({percent}%)</span></p>
        </div>
      );
    }
    return null;
        ...
        <div className="p-4 m-4 bg-slate-800 rounded-xl border border-slate-700 relative select-none">
          ...
          >
            ...
            <div className="overflow-hidden flex-1">
              ...
              <p className="text-xs text-slate-400 truncate" title={authUser?.email}>
                 {authUser?.email}
              </p>
            </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-in fade-in duration-300">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div onClick={() => openDetailModal('total')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-emerald-300 hover:shadow-md transition-all cursor-pointer group">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="p-2 rounded-lg bg-emerald-100 text-emerald-600"><Wallet className="w-6 h-6" /></div>
                </div>
                <p className="text-sm text-slate-400 flex items-center gap-1">Click to view breakdown <ArrowUpRight className="w-3 h-3"/></p>
              </div>

              <div onClick={() => openDetailModal('liquid')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-300 hover:shadow-md transition-all cursor-pointer group">
                <div className="flex justify-between items-start mb-4">
                  <div>
                  ...
                  <div className="p-2 rounded-lg bg-blue-100 text-blue-600"><TrendingUp className="w-6 h-6" /></div>
                </div>
                <p className="text-sm text-slate-400 flex items-center gap-1">Click to view breakdown <ArrowUpRight className="w-3 h-3"/></p>
              </div>

              <div onClick={() => openDetailModal('tangible')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-violet-300 hover:shadow-md transition-all cursor-pointer group">
                <div className="flex justify-between items-start mb-4">
                  <div>
                  ...
                  <div className="p-2 rounded-lg bg-violet-100 text-violet-600"><Building className="w-6 h-6" /></div>
                </div>
                <p className="text-sm text-slate-400 flex items-center gap-1">Click to view breakdown <ArrowUpRight className="w-3 h-3"/></p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <h3 className="text-lg font-bold text-slate-900 mb-6">Net Worth Growth & Composition</h3>
              <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={timelineData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                    <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12 }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12 }} tickFormatter={(val) => currencySettings.display === 'USD' ? `$${(val/currencySettings.usdRate/1000).toFixed(1)}k` : `${val/1000}k`} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    ...
                    {entries.length === 0 && <tr><td colSpan="6" className="p-8 text-center text-slate-500 font-medium">No logs found. Add your first asset to get started!</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="h-64 mb-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={[{ name: 'Liquid Assets', value: currentMetrics.liquid, color: '#3b82f6' }, { name: 'Non-Liquid Assets', value: currentMetrics.nonLiquid, color: '#94a3b8' }]} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <h3 className="text-lg font-bold text-slate-900 mb-6 text-center">Asset Composition</h3>
                <div className="h-64 mb-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              ...
              <div className="flex gap-4 items-end">
                <div className="flex-1">
                  ...
                  <input type="date" value={searchParams.start} onChange={e => setSearchParams({...searchParams, start: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg" />
                </div>
                <div className="flex-1">
                  <label className="block text-sm font-medium text-slate-700 mb-1">End Date</label>
                  <input type="date" value={searchParams.end} onChange={e => setSearchParams({...searchParams, end: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex gap-4">
                  <button onClick={() => handleUpdateSettings({...currencySettings, display: 'BDT'})} className={`flex-1 py-3 rounded-xl border-2 font-bold transition-all ${currencySettings.display === 'BDT' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500'}`}>৳ BDT</button>
                  <button onClick={() => handleUpdateSettings({...currencySettings, display: 'USD'})} className={`flex-1 py-3 rounded-xl border-2 font-bold transition-all ${currencySettings.display === 'USD' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500'}`}>$ USD</button>
                </div>
              </div>
              {currencySettings.display === 'USD' && (
                <div className="pt-4 border-t border-slate-100 animate-in slide-in-from-top-2">
                  ...
                  <input type="number" value={currencySettings.usdRate} onChange={(e) => handleUpdateSettings({...currencySettings, usdRate: Number(e.target.value) || 1})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-lg font-medium"/>
                </div>
              )}
            </div>
          </div>
            ...
            <form onSubmit={handleSaveEntry} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input type="text" required placeholder="e.g., Cash, Gold, Apartment 4B" value={formData.assetName} onChange={e => setFormData({...formData, assetName: e.target.value})} className="w-full px-4 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" list="asset-suggestions"/>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" required min="0" value={formData.value} onChange={e => setFormData({...formData, value: e.target.value})} className="w-full pl-8 pr-4 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"/>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Date Logged</label>
                  <input type="date" required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full px-4 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Liquidity</label>
                  <select value={formData.liquidity} onChange={e => setFormData({...formData, liquidity: e.target.value})} className="w-full px-4 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
                ...
                <div>
                  ...
                  <select value={formData.tangibility} onChange={e => setFormData({...formData, tangibility: e.target.value})} className="w-full px-4 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
              ...
              <ul className="space-y-4">
                {detailModal.data.map((asset, index) => (
                  <li key={index} className="flex justify-between items-center border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                    ...
                    <span className="font-bold text-slate-900 bg-slate-100 px-3 py-1 rounded-lg">{formatCurrency(asset.value)}</span>
                  </li>
                ))}
              </ul>
            </div>
